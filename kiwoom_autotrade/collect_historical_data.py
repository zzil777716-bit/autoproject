"""
========================================================================================
📊 [DATA COLLECTOR] 삼성전자(005930) 3분봉 / 5분봉 / 15분봉 과거 데이터 전수 수집기
수집 기간: 2025년 8월 ~ 현재
저장 형식: SQLite DB (data/market_data.sqlite) + CSV (data/*.csv) + Parquet (data/*.parquet)
========================================================================================
"""
...
def main():
    print("=" * 75)
    print(">> [SAM-BOT Data Collector] 삼성전자(005930) 멀티 분봉 데이터 수집 시작")
    ...
    for tf in timeframes:
        print("\n" + "=" * 75)
        print(f">> [Step 2-{tf}] 삼성전자({code}) {tf}분봉 과거 데이터 수집 시작...")
    ...
    # 3. 수집 완료 리포트 출력
    print("\n" + "=" * 75)
    print(">> [Result] 🎉 모든 분봉 데이터 수집 및 저장이 성공적으로 완료되었습니다!")
    print("=" * 75)
    ...
    print("=" * 75)
    print("\n>> 💡 저장된 데이터 파일 위치:")
    print(f"   • SQLite DB : {os.path.abspath('data/market_data.sqlite')}")
