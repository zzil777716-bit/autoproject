"""
Time-Series Storage & Fast Resampling Engine
...
class TimeSeriesDB:
    ...
    def save_timeframe_bars(self, code: str, timeframe: str, df_bars: pd.DataFrame):
        """
        특정 타임프레임(3m, 5m, 15m, 1m)의 분봉 데이터를 SQLite DB에 적재하고
        데이터 분석을 위해 CSV 및 Parquet 파일로 동시 익스포트.
        """
        if df_bars.empty:
            return
        ...
        # 2. CSV 및 Parquet 파일 익스포트 (데이터 분석용)
        ...
        print(f">> [TimeSeriesDB] ✅ {code} {tf_name} 데이터 저장 완료: 총 {len(df_bars):,d}개 봉 -> [DB, CSV, Parquet]")

    def load_timeframe_bars(self, code: str, timeframe: str, start_date: str = "2025-08-01") -> pd.DataFrame:
        """저장된 특정 타임프레임 분봉 데이터를 DataFrame으로 로드"""
        tf_name = timeframe.lower().replace("min", "m").replace("t", "m")
        table_name = f"bars_{tf_name}"
    ...
    def load_recent_bars(self, code: str, limit: int = 500) -> pd.DataFrame:
        """최신 N개의 1분봉 데이터를 DataFrame으로 로드"""
        with sqlite3.connect(self.db_path) as conn:
            query = """
                SELECT timestamp, open, high, low, close, volume
                FROM bars_1m
