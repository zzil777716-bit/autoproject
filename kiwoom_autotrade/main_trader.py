"""
========================================================================================
🚀 [SAM-BOT v2.4] 삼성전자(005930) 키움증권 1주 모의투자 다중 주기 자동매매 엔진
Active Strategy: [전략 2: 변동성 수축 및 수급 반등형 (MTF-Squeeze & Divergence Reversal)]
...
from core.strategy_mtf import MTFStrategyEngine, StrategySignal
...
class MainTradingSystem:
    def __init__(self):
        print("=" * 80)
        print(">> [System] 삼성전자(005930) [전략 2: 변동성 수축 및 수급 반등형] 1주 모의투자 엔진 초기화...")
        print(f">> 계좌번호: {config.ACCOUNT_NO} | 주문단위: {config.DEFAULT_TRADE_QTY}주 고정 | 일일 한도: {config.MAX_DAILY_TRADES}회")
        print("=" * 80)
        ...
        self.strategy_engine = MTFStrategyEngine()
    ...
    def start(self):
        ...
        if self.api:
            ...
            if not success:
                print(">> [Error] 로그인 실패로 시스템을 종료합니다.")
                sys.exit(1)
        ...
        if not df_history.empty:
            ...
            print(f">> [System] ✅ 과거 분봉 {len(df_history)}개 프리로드 완료 (지표 웜업 완료!)")
        ...
        print("\n>> [System] 🚀 삼성전자 전략 2 실시간 자동매매 엔진이 정상 가동되었습니다!")
    ...
    def _sync_stock_info(self):
        ...
        try:
            ...
            if stock_data:
                ...
                print(f">> [System] 📊 삼성전자 시세 동기화: {self.current_price:,.0f}원 ({self.price_diff_rate:+.2f}%)")
        except Exception as e:
            pass
    ...
    def _on_system_heartbeat(self):
        ...
        if self.current_price > 0 and len(df_15m) >= 20:
            ...
            if time.time() - self.last_dashboard_time >= 5.0:
                ...
                if "SQUEEZE" in signal.state_15m and not signal.should_enter:
    ...
    def _execute_entry(self, signal: StrategySignal, now: datetime):
        trade_id = f"SAM-{now.strftime('%Y%m%d%H%M%S')}"
        order_qty = self.risk_manager.calculate_order_qty(self.current_price)
        print(f"\n🚀 [ENTRY TRIGGER] ID:{trade_id} | {signal.reason} | 수량: {order_qty}주")
    ...
    def _handle_risk_action(self, action: RiskAction):
        ...
        if action.action_type in ["FULL_SELL", "STOP_LOSS"]:
            print(f"\n🔴 [EXIT EXECUTED] 사유: {action.reason} | 수량: {action.qty}주")
            if self.api:
                self.api.send_order(
                    rq_name="MTF_SQUEEZE_EXIT",
                    screen_no=config.SCREEN_NO_ORDER,
                    acc_no=config.ACCOUNT_NO,
                    order_type=2,
                    code=config.STOCK_CODE,
    ...
    def _render_tui_dashboard(self, now: datetime, can_trade: bool, block_reason: str):
        ...
        tui = f"""
┌─ [SAM-BOT v2.4] 삼성전자(005930) [전략 2: 변동성 수축/수급 반등형] 1주 모의투자 모드 ───────────┐
│ 현재시간: {now.strftime('%Y-%m-%d %H:%M:%S')} │ 삼성전자: {self.current_price:>9,.0f}원 ({self.price_diff_rate:>+6.2f}%) │ 체결강도: {self.realtime_intensity:>5.1f}% │
├────────────────────────────────────────────────────────────────────────────────────────────────────────┤
...
├────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ [2] 다중 주기 상태 (MTF Cascading Status - Strategy 2)                                                │
│  • 15분봉 (Squeeze): {state_15m:<55} │
│  • 05분봉 (Diverg) : {state_5m:<55} │
│  • 03분봉 (Trigger): {state_3m:<55} │
├────────────────────────────────────────────────────────────────────────────────────────────────────────┤
