...
class ThemeScanner:
    ...
    def scan_market_themes(self, api_instance=None) -> Dict[str, Any]:
        """HTS 4종 데이터 수집 및 1달 단위 통합 엑셀 워크북에 날짜별 시트 누적 업데이트"""
        ...
        # -------------------------------------------------------------
        # SECTION 1: [0659] 3대 주도 테마 & 9대 대장주
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # SECTION 2: [0198] 키움 실시간 조회수/검색 급등 20종목
        # -------------------------------------------------------------
        ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=8)
        ...
        # -------------------------------------------------------------
        # SECTION 3: [0184] 당일 거래대금 상위 순수 KOSPI 20종목
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # SECTION 4: [0184] 당일 거래대금 상위 순수 KOSDAQ 20종목
        # -------------------------------------------------------------
        ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=7)
        ...
        # 구글 드라이브(G:\내 드라이브\Antigravity\테마)로 자동 동기화
